...
class Application extends mali_1.default {
    ...
    register(name, service, hooks = {}) {
        ...
        for (const key in methodMap) {
            this.use(key, ...lodash_2.concat(lodash_2.get(hooks, 'before.all', []), lodash_2.get(hooks, `before.${methodMap[key]}`, [])), async (ctx) => await service[methodMap[key]](ctx), ...lodash_2.concat(lodash_2.get(hooks, 'after.all', []), lodash_2.get(hooks, `after.${methodMap[key]}`, [])));
        }
    }
}
